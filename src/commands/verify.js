...
export default {
    ...
    async execute(interaction) {
        ...
        // Already verified?
        if (process.env.VERIFIED_ROLE_ID && interaction.member.roles.cache.has(process.env.VERIFIED_ROLE_ID)) {
            return interaction.reply({
                embeds: [buildEmbed({
        ...
        try {
            ...
            return interaction.reply({
                embeds: [buildEmbed({
                    ...
                    description: "I couldn't send you a direct message. Please enable DMs from server members in your Privacy Settings and try again.",
                    color: COLORS.ERROR,
                })],
                ephemeral: true,
            });
